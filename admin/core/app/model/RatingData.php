...
class RatingData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (rating,comment,product_id,client_id,created_at) ";
		$sql .= "value (\"$this->rating\",\"$this->comment\",$this->product_id,\"$this->client_id\",$this->created_at)";
...
// partiendo de que ya tenemos creado un objecto RatingData previamente utilizamos el contexto
	...
	public static function getPublicsByProductId($id){
		$sql = "select * from ".self::$tablename." where product_id=$id and status_id=1 order by created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new RatingData());
	}
