...
<form class="form-horizontal" role="form" enctype="multipart/form-data" method="post" action="index.php?action=updateproduct">
  ...
  <input type="text" class="form-control" placeholder="Precio" value="<?php echo $product->price; ?>" required name="price">
    ...
    <label for="inputEmail1" class="col-lg-2 control-label">Precio de Oferta</label>
    <div class="col-lg-4">
      <div class="input-group">
  <span class="input-group-addon"><?php echo $coin; ?></span>
  <input type="text" class="form-control" placeholder="Precio de Oferta" value="<?php echo $product->price_offer; ?>"  name="price_offer">
</div>    </div>
...
<?php $subcategories = CategoryData::getAllByCatId($product->category_id);
?>
<select name="subcategory_id" id="subcategory_id" class="form-control" required>
<option value="">-- SELECCIONE SUBCATEGORIA --</option>
<?php foreach($subcategories as $cat):?>
<option value="<?php echo $cat->id; ?>" <?php if($product->subcategory_id==$cat->id){ echo "selected";} ?>><?php echo $cat->name; ?></option>
<?php endforeach; ?>
</select>
...
<script type="text/javascript">
  $(document).ready(function(){
    $("#category_id").change(function(){
      ...
      $.get("./?action=getsubcats","cat_id="+$("#category_id").val(), function(data){
